...
export class RectangleFish extends Fish {
  private step: number = 0;      // 0=rechts, 1=runter, 2=links, 3=hoch
  private distance: number = 0;
  private sideLength: number = 150;

  update(): void {
    const speed = Math.abs(this.speedX);
    ...
    if (this.distance >= this.sideLength) {
      this.distance = 0;
...
export class CircleFish extends Fish {
  ...
  constructor(
    aquarium: HTMLElement,
    src: string,
    startX: number,
    startY: number,
    speed: number
  ) {
    super(aquarium, src, startX, startY, speed, 0);
    this.centerX = startX;
    this.centerY = startY;
  }
